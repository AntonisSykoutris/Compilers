...
void remove_str_quotes(char* ptr){
    char *tmp = ptr;
    char *next = ptr + 1;
...
unsigned libfuncs_newused(const char* s){
    ...
    if(totalnamedLibFuncs == namedLibFuncsSize){
...
void print_bc(FILE* stream){
    int i;
    size_t size;
    unsigned inst_size = currInst-1;
    /* magic number */
    long magic_number = 384838503862;
    fwrite(&magic_number, sizeof(long), 1, stream);

    /* string consts array*/
    fwrite(&totalStringConsts, sizeof(unsigned int), 1, stream);
    for(i = 0; i < totalStringConsts; i++){
    ...
    /* numConsts array */
    fwrite(&totalNumConsts, sizeof(unsigned int), 1, stream);
    for(i = 0; i < totalNumConsts; i++){
        fwrite(&numConst[i], sizeof(double), 1, stream);
    }

     /* userFuncs array */
    fwrite(&totalUserFuncs, sizeof(unsigned int), 1, stream);
    for(i = 0; i < totalUserFuncs; i++){
        ...
        fwrite(userFuncs[i].id, sizeof(double), size, stream);
    }
